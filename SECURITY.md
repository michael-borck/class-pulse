...
- **Image uploads** — presenter-only (login + session-ownership checked) and rate-limited (60/h). Every upload is decoded and **re-encoded to JPEG** with Pillow, which rejects anything that isn't a valid raster and strips metadata — defeating SVG/script/polyglot files (SVG is never stored or served). A decompression-bomb pixel cap (`IMAGE_MAX_PIXELS`) and a size cap (`UPLOAD_MAX_BYTES`, default 8 MB; the 256 KB body limit is re-imposed on all other routes) bound resource use; images are downscaled to `IMAGE_MAX_DIM`. Files are stored in the persisted `instance/uploads/<session_id>/` tree under server-assigned UUID names (client filename never trusted) and served same-origin; they're deleted when the session is hard-deleted.
